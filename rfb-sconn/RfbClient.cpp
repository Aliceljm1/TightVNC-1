...
void RfbClient::execute()
{
	...
	try {
		...
		remoteProcess = new RemoteProcessRequestHandler(&codeRegtor, &output, m_desktop, this, m_log);
...
{
	// FIXME
	UpdateContainer newContainer(*updateContainer);
	newContainer.screenSizeChanged = TRUE;

	m_updateSender->newUpdates(&newContainer, cursorShape);
}
